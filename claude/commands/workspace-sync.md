...
- `CONFLICT <repo>: …` — the merge is **left in progress** in `$WS/modules/<repo>`.
...
(`$WS/modules/<repo>`). List the unmerged files:
```bash
git -C "$WS/modules/<repo>" diff --name-only --diff-filter=U
```
...
```bash
git -C "$WS/modules/<repo>" diff --diff-filter=U -- <file>
```
...
```bash
git -C "$WS/modules/<repo>" add <resolved files>
git -C "$WS/modules/<repo>" commit --no-edit
```
